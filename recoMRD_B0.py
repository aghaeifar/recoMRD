...
class recoMRD_B0(recoMRD):
    ...
    def unwrap_b0(self):
        ...
        #b0 = np.take(self.img_b0, crep, axis=self.dim_info['rep']['ind'])
        ...
        handle.unwrap_b0(self.img_b0, self.img_b0_uw, *b0_size) # 3D and 4D input

    def create_mask(self):
        self.img_mask = np.zeros(self.img_mag.shape, dtype=np.float32, order='F')
        ...
        handle.runBET.argtypes = [np.ctypeslib.ndpointer(np.float32, ndim=self.img_mag.ndim, flags='F'),
                                  np.ctypeslib.ndpointer(np.float32, ndim=self.img_mask.ndim, flags='F'),
                                  ctypes.c_int, ctypes.c_int, ctypes.c_int]

        handle.runBET(self.img_mag, self.img_mask, *mask_size) # 3D input
